...
public class LoginUser extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnSignup.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                startActivity(new Intent(LoginUser.this, SignUpActivity.class));
            }
        });
                        ...
                        .addOnCompleteListener(LoginUser.this, new OnCompleteListener<AuthResult>() {
                            ...
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                ...
                                if (!task.isSuccessful()) {
                                    ...
                                    DatabaseReference mDatabase2 = FirebaseDatabase.getInstance().getReference().child("users").child(auth.getCurrentUser().getUid()).child("firstLogin");
                                    mDatabase2.addValueEventListener(new ValueEventListener() {
                                        @Override
                                        public void onDataChange(DataSnapshot dataSnapshot) {
                                            String message = dataSnapshot.getValue(String.class);
                                            if(message == null){
                                                RegisterUserInFireBaseDatabase();
    ...
    private void RegisterUserInFireBaseDatabase() {
        DatabaseReference mDatabase = FirebaseDatabase.getInstance().getReference().child("users");
        DatabaseReference currentUserDB = mDatabase.child(auth.getCurrentUser().getUid());


        currentUserDB.child("name").setValue(auth.getCurrentUser().getDisplayName());
        currentUserDB.child("registration_id").setValue("0");
        currentUserDB.child("firstLogin").setValue("0");
        currentUserDB.child("nickname").setValue(auth.getCurrentUser().getDisplayName());
